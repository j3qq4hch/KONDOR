...
// ── Eagle settings file ───────────────────────────────────────────────────────
function eagleSettingsArgs(): string[] {
...
const glbWatchers = new Map<string, ReturnType<typeof watch>>();

function watchBrd(brdPath: string): void {
  if (watchers.has(brdPath)) return;
  try {
...
function unwatchBrd(brdPath: string): void {
  watchers.get(brdPath)?.close();
  watchers.delete(brdPath);
}

function watchGlb(glbPath: string): void {
  if (glbWatchers.has(glbPath)) return;
  try {
...
function unwatchGlb(glbPath: string): void {
  glbWatchers.get(glbPath)?.close();
  glbWatchers.delete(glbPath);
}

// ── ConBut / Pinout windows ───────────────────────────────────────────────────
let conbutWin: BrowserWindow | null = null;
let pinoutWin: BrowserWindow | null = null;
let storedConButLayout: unknown = null;

function setConIdInXml(xml: string, refDes: string, newValue: string): string {
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('board:getMtime', (_e, filePath: string) => {
    try { return statSync(filePath).mtimeMs; }
    catch { return null; }
  });

  // Regenerate GLB via eaglecon
  ipcMain.handle('board:update', async (_e, brdPath: string) => {
    const settings = readSettings();
    const eagleBin = settings.eagleBinPath;
    if (!eagleBin) return { ok: false, error: 'Eagle bin path not set in settings.' };

    const eaglecon = join(eagleBin, 'eaglecon.exe');
    if (!existsSync(eaglecon)) return { ok: false, error: `eaglecon.exe not found at: ${eaglecon}` };

    const mtimeBefore = (() => {
      const dir  = dirname(brdPath);
      const stem = basename(brdPath, extname(brdPath));
      try { return statSync(join(dir, stem + '.glb')).mtimeMs; } catch { return 0; }
    })();

    return new Promise<{ ok: boolean; glbPath?: string; brdContent?: string; brdMtime?: number; error?: string }>(resolve => {
      const eagleconCmd = settings.eagleconCmd || "run export3D_raw.ulp '400'; UNDO; QUIT";
      const settingsArgs = eagleSettingsArgs();
      const proc = spawn(
        eaglecon,
        [...settingsArgs, '-C', eagleconCmd, brdPath],
        { detached: false }
      );
      proc.on('close', () => {
        const dir     = dirname(brdPath);
  ...
  ipcMain.handle('board:openInEagle', async (_e, brdPath: string) => {
    const settings = readSettings();
    const eagleBin = settings.eagleBinPath;
    if (!eagleBin) return { ok: false, error: 'Eagle bin path not set in settings.' };

    const eagle = join(eagleBin, 'eagle.exe');
    if (!existsSync(eagle)) return { ok: false, error: `eagle.exe not found at: ${eagle}` };
  ...
  ipcMain.handle('board:unwatch', (_e, brdPath: string) => {
    unwatchBrd(brdPath);
  });

  // GLB file watching (for pure GLB model entities)
  ipcMain.handle('glb:watch',   (_e, glbPath: string) => watchGlb(glbPath));
  ipcMain.handle('glb:unwatch', (_e, glbPath: string) => unwatchGlb(glbPath));

  // Load BRD from known path without dialog (used for device restore)
  ipcMain.handle('board:loadBrd', (_e, brdPath: string) => {
    if (!existsSync(brdPath)) return null;
    const dir      = dirname(brdPath);
    const stem     = basename(brdPath, extname(brdPath));
    const glbPath  = join(dir, stem + '.glb');
    const brdMtime = statSync(brdPath).mtimeMs;
  ...
  ipcMain.handle('device:save', async (_e, { data, filePath }: { data: string; filePath?: string }) => {
    let target = filePath;
    if (!target) {
      ...
      if (canceled || !chosen) return { ok: false };
      target = chosen;
    }
    try {
  ...
  ipcMain.handle('device:loadFile', (_e, filePath: string) => {
    try {
  ...
  // Export scene as GLB
  ipcMain.handle('scene:export', async (_e, buf: ArrayBuffer) => {
  ...
  ipcMain.handle('settings:set', (_e, data: Record<string, string>) => {
    writeSettings(data); return true;
  });

  // ── ConBut ──────────────────────────────────────────────────────────────────
  ipcMain.handle('conbut:open', (_e, payload: unknown) => {
  ...
  ipcMain.handle('conbut:update-layout', (_e, layout: unknown) => {
  ...
  ipcMain.handle('brd:set-conid', (_e, { brdPath, refDes, value }: { brdPath: string; refDes: string; value: string }) => {
    try {
